# YCLIENTS test 1 — быстрое закрытие popup

Тестовый вариант: top-level POST выполняется в служебном окне, которое закрывается примерно через 0,95 секунды на desktop и 1,4 секунды на мобильном устройстве.

## Эксперимент YCLIENTS

1. Нажмите `iFrame` на экране CRM Data.
2. Служебное окно открывается из пользовательского клика, отправляет форму на `auth/login/1`, теряет фокус и закрывается после короткой паузы.
3. Расписание загружается в iFrame.

Рекомендуется сравнить 950/1400 мс с более длинными задержками, если cookie не успевают сохраниться.

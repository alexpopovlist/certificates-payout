# YCLIENTS test 6 — браузерное расширение

Тестовый вариант для Chrome/Edge desktop: расширение выполняет cross-origin login fetch с host permissions и проверяет cookies через chrome.cookies.

## Эксперимент YCLIENTS

Перед тестом установите распакованное расширение из папки `browser-extension`. При отсутствии расширения приложение покажет таймаут; затем можно использовать кнопку `Popup fallback`.

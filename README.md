# YCLIENTS test 3 — скрытая форма в iFrame

Тестовый вариант без popup: POST-форма отправляется в невидимый iFrame, после чего расписание загружается в видимый iFrame.

## Эксперимент YCLIENTS

Этот вариант не открывает popup. Если браузер блокирует third-party cookies, расписание в видимом iFrame останется неавторизованным. Для сравнения в модальном окне оставлена кнопка `Popup fallback`.

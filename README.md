# YCLIENTS test 5 — серверный reverse proxy

Экспериментальный reverse proxy: сервер хранит cookies YCLIENTS в сессии и загружает страницы YCLIENTS через same-origin маршрут приложения.

## Эксперимент YCLIENTS

Вариант добавляет маршрут `/api/crm-data/yclients-proxy/*`. Он передаёт сохранённые на сервере cookies YCLIENTS и переписывает часть ссылок на same-origin proxy.

Это прототип: сложные SPA-запросы, WebSocket, CSP, динамически построенные абсолютные URL и изменения разметки YCLIENTS могут потребовать дополнительного переписывания. Не использовать в production без согласования с YCLIENTS и проверки безопасности.


---

# Исходная документация приложения
...
PROFILE_NOTIFICATION_CHANNELS_URL=https://partner-wowlife.ru/restapi/profile.getNotificationChannels
PROFILE_CABINET=
...
```http
POST https://partner-wowlife.ru/restapi/profile.getNotificationChannels
```

Payload формируется из текущей сессии авторизованного партнёра:

```json
{"contactId":"301","token":"..."}
```

Ответ вида `{"result":{"channels":[{"id":472,"name":"TG"}]}}` отмечает соответствующий канал активным на экране `/profile`.
